...
# empty args
if [[ $# -eq 0 ]] ; then
    echo -e "\n \033[0;31m Please Enter A Deposit Amount In Lovelace \033[0m \n";
    exit
fi

# arg equals 0
if [[ ${1} -eq 0 ]] ; then
    echo -e "\n \033[0;31m Deposit Must Be Greater Than Zero \033[0m \n";
    exit
fi
...
variable=${1}; jq --argjson variable "$variable" '.fields[0].fields[0].int=$variable' data/redeemer/deposit-redeemer.json > data/redeemer/deposit-redeemer-new.json
mv data/redeemer/deposit-redeemer-new.json data/redeemer/deposit-redeemer.json
...
FEE=$(${cli} transaction build \
    ...
    --spending-reference-tx-in-redeemer-file data/redeemer/deposit-redeemer.json \
